...
const { FileNotFoundError, CLOUD_PROVIDERS, resolvePath, PicoS3, getFileKey} = require("./dist");
...
test.serial('Get the presigned upload URL', async t => {
...
test.serial('Make sure non existent file throws FileNotFoundError', async t => {
  const filePath = getFileKey(options) || `/${resolvePath(options)}`
	const error = await t.throwsAsync(async () => {
        return await p3.getObjectDataUrl(options)
	}, {instanceOf: FileNotFoundError});
	t.is(error.message, `File ${filePath} not found in ${p3.getProvider()}`);
});
